...
module.exports.run = async function ({ api, event, Users, args, client }) {
  ...
  switch (args[0]) {
    case 'random':
      ...
      rc.isRandom = true;
      o("Bạn đã chọn chế độ tìm kiếm ngẫu nhiên.\nBạn có muốn ẩn giấu thân phận?\n\nReply với số 0 để ẩn, 1 để hiện", (_, i) => client.handleReply.push({ type: 'incognitoRootRandomHead', name: this.config.name, author: threadID, messageID: i.messageID, data: rc }));
      break;
    case 'reject':
      var data = require(rcp);
    ...
    case 'accept':
      ...
      if (data.some(e => e.head == threadID && e.isWaiting)) {
        ...
        o("Bạn đã chấp nhận kết nối với người liên hệ.\nBạn có muốn ẩn giấu thân phận?\n\nReply với số 0 để ẩn, 1 để hiện", (_, i) => client.handleReply.push({ type: 'incognitoHead', name: this.config.name, author: threadID, messageID: i.messageID, data: ac }))
      }
      break;
    case 'end':
    case 'leave':
    ...
    default:
      ...
      o(`Bạn đã yêu cầu ${args[0]} kết nối trò chuyện.\nBạn có muốn ẩn giấu thân phận?\n\nReply với số 0 để ẩn, 1 để hiện`, (_, i) => client.handleReply.push({ type: 'incognitoRoot', name: this.config.name, author: threadID, messageID: i.messageID, data: rc }));
      break;
  }
}

module.exports.handleReply = async function ({ handleReply, api, event, Users, client }) {
  ...
  switch (handleReply.type) {
    ...
    case 'incognitoHead':
      ...
      if (body == 1) {
        ...
        o(`Bạn đã chọn hiện thân phận.\nĐể kết thúc cuộc trò chuyện hãy nhập lệnh: .rc end`);
      } else if (body == 0) {
    ...
    case 'incognitoRoot':
      ...
      s((rc.rootIsIncognito ? "Có 1 người liên hệ ẩn danh muốn trò chuyện với bạn" : `Một người tên ${rootName} - ${fbUrl} muốn trò chuyện với bạn.`) + `\nNếu muốn từ chối hãy nhập lệnh: .rc reject\nNếu muốn chấp nhận hãy nhập lệnh: .rc accept`, rc.head, (er, i) => {
        if (er) o("Đã có lỗi xảy ra khi cố kết nối với người liên hệ.\nVui lòng thử lại sau.");
        else {
    ...
    if (rcData.some(e => (e.head == threadID || e.root == threadID))) {
      ...
      if (data.isStarted == true) {
        ...
        if (senderIncognito == false) {
          let data = await api.getUserInfo(sender);
          var name = await data[sender].name;
