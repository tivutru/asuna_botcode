...
module.exports.event = async function({ api, event, Currencies, Users, client }) {
	...
	if (level > curLevel && level != 1) {
		...
		var messsage = (typeof threadData.customRankup == "undefined") ? msg =`Trình độ chém gió của ${nameUser} đã đạt tới {level}` : msg = threadData.customRankup,
			arrayContent;
		...
		if (existsSync(__dirname + `/cache/rankup/vip.gif`)) arrayContent = { body: messsage, attachment: createReadStream(__dirname + `/cache/rankup/vip.gif`), mentions: [{ tag: nameUser, id: senderID }] };
